...
if (!lsaToken || !userData || userData.dashboard !== 'admin' || userData.level !== 'district') {
...
// ── Role helpers ─────────────────────────────────────────────────────────────
const isDistrictHead     = userData && !userData.color && !userData.is_functional;
const isColoredDistrict  = userData && !!userData.color;
const isFunctionalDistrict = userData && !userData.color && !!userData.is_functional;

// ── Page init ────────────────────────────────────────────────────────────────
let allEvents = [], allReports = [], allUsers = [];
let allGroupsRef = [], allDistrictsRef = [];
...
(async function init() {
  if (userData) {
    ...
    document.getElementById('user-role-label').textContent =
      (userData.role_title || 'District Commissioner') + colorTag;
  }

  // My District nav section: visible for head/admin and colored; hidden for functional
  if (!isFunctionalDistrict) {
  ...
  const [stats, groups, districts] = await Promise.all([
    api('GET', '/api/stats'),
    api('GET', '/api/groups'),
  ...
  if (stats && stats.pending_requests > 0) {
    ...
    ['req-badge','header-badge','approve-reqs-badge'].forEach(id => {
...
function renderUsersTable(users) {
  ...
  if (!display.length) {
    tbody.innerHTML = '<tr><td colspan="8" class="text-muted text-sm" style="padding:1rem;">No users in this category.</td></tr>';
    return;
  }
  tbody.innerHTML = display.map(u => `
    <tr style="${!u.editable ? 'opacity:0.8;' : ''}">
      ...
      <td style="font-size:0.82rem;">${u.group_code ? escHtml(u.group_code) : (u.group_name ? escHtml(u.group_name) : '—')}</td>
      <td><span class="badge ${u.dashboard === 'admin' ? 'badge-danger' : u.dashboard === 'leader' ? 'badge-primary' : 'badge-info'}"
               style="text-transform:capitalize;">${u.dashboard}</span></td>
      <td>
        ${u.editable ? `
...
function openAddUserModal() {
  ...
  document.getElementById('um-dashboard').value = 'member';
  document.getElementById('um-level').value = 'member';
...
function openEditUserModal(userId) {
  ...
  document.getElementById('um-dashboard').value = u.dashboard;
  document.getElementById('um-level').value = u.level;
  document.getElementById('um-color').value = u.color || '';
  document.getElementById('um-role-title').value = u.role_title || '';
  document.getElementById('um-password').value = '';
  populateGroupDropdown(u.group_id);
  document.getElementById('user-modal').classList.add('open');
}
...
function populateGroupDropdown(selectedGroup) {
  const districtGroups = allGroupsRef.filter(g => g.district_id === userData.district_id);
  const gSel = document.getElementById('um-group');
  gSel.innerHTML = '<option value="">— Select group —</option>' +
    districtGroups.map(g => `<option value="${g.id}" ${g.id === selectedGroup ? 'selected' : ''}>${escHtml(g.code)} – ${escHtml(g.name || '')}</option>`).join('');
}
...
async function saveUser() {
  const body = {
    ...
    dashboard: document.getElementById('um-dashboard').value,
    level: document.getElementById('um-level').value,
    color: document.getElementById('um-color').value || null,
    role_title: document.getElementById('um-role-title').value.trim() || null,
    group_id: parseInt(document.getElementById('um-group').value) || null,
  ...
  if (res && res.id) {
    toast(editingUserId ? 'User updated.' : 'User created.', 'success');
    closeUserModal();
    allUsers = await api('GET', '/api/users') || [];
    renderUsersTable(allUsers);
  } else {
    toast(res?.error || 'Failed to save user.', 'danger');
  }
}
...
function renderPendingEvents() {
  const el = document.getElementById('pending-reqs-list');
  const pending = allEvents.filter(e => e.status === 'pending');
  if (!pending.length) {
    el.innerHTML = '<div class="card"><div class="card-body text-muted text-sm">No pending requests at district level.</div></div>';
    return;
  }
    ...
    <div class="card">
      <div class="card-body">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;margin-bottom:0.75rem;">
          <div>
            <div class="font-semibold" style="font-size:1rem;">${escHtml(e.title)}</div>
            <div class="text-muted text-sm mt-1">
              Submitted by <strong>${escHtml(e.submitter_name || '—')}</strong>
              ${e.submitter_color ? `· <span style="text-transform:capitalize;">${e.submitter_color} branch</span>` : ''}
        ...
        ${e.description ? `<p class="text-sm" style="color:var(--text-secondary);margin-bottom:0.75rem;">${escHtml(e.description)}</p>` : ''}
        <div class="flex gap-2 flex-wrap" style="margin-bottom:0.75rem;">
          <span class="badge badge-neutral">Needs ${e.required_approval_level.toUpperCase()} approval</span>
          <span class="badge badge-neutral">Currently at: ${e.current_level}</span>
          <span class="badge badge-neutral">Submitted ${fmtDate(e.created_at)}</span>
        </div>
        <div class="flex gap-2">
          <button class="btn btn-success" onclick="promptAction('approve', ${e.id})">
            ✓ ${e.current_level === e.required_approval_level ? 'Approve' : 'Approve & Forward to GC'}
          </button>
          <button class="btn btn-danger" onclick="promptAction('reject', ${e.id})">✕ Reject</button>
        </div>
      </div>
    </div>`).join('');
}
...
function renderAllEventsTable() {
  const tbody = document.getElementById('all-reqs-tbody');
  if (!allEvents.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="text-muted text-sm" style="padding:1rem;">No requests.</td></tr>';
    return;
  }
  tbody.innerHTML = allEvents.map(e => `
    <tr>
      <td class="td-name">${escHtml(e.title)}</td>
      <td>${escHtml(e.submitter_name || '—')}</td>
      <td>${e.location ? escHtml(e.location) : '—'}</td>
      <td>${e.start_date || '—'}</td>
      <td><span class="badge badge-neutral" style="text-transform:uppercase;">${e.required_approval_level}</span></td>
      <td><span class="badge ${STATUS_BADGE[e.status] || 'badge-neutral'}"
               style="text-transform:capitalize;">${e.status}</span></td>
      <td>
        ${e.status === 'pending' ? `
          <div class="flex gap-2">
            <button class="btn btn-sm btn-success" onclick="promptAction('approve',${e.id})">✓</button>
            <button class="btn btn-sm btn-danger"  onclick="promptAction('reject', ${e.id})">✕</button>
...
async function executeAction() {
  ...
  if (type === 'approve') {
    ...
    if (res && !res.error) toast(res.status === 'approved' ? 'Request approved.' : 'Request forwarded to GC.', 'success');
  } else if (type === 'reject') {
    ...
    if (res && !res.error) toast(res.status === 'approved' ? 'Report approved.' : 'Report forwarded to GC.', 'success');
    ...
    <div class="request-card" id="inbox-card-${e.id}">
      <div class="request-card-header">
        <div style="flex:1;min-width:0;">
          ...
          <div class="meta">${escHtml(e.submitter_name || '—')} · ${fmtDate(e.created_at)}</div>
        </div>
      ...
      <div class="flex gap-2" style="margin-top:0.5rem;">
        <button class="btn btn-success btn-sm" onclick="promptAction('approve', ${e.id})">✅ Approve</button>
...
function renderInboxReqHistory() {
  ...
  populateHistoryFilters(inboxReqHistory, 'hist-req-district-filter', 'hist-req-group-filter');
...
function filterInboxReqHistory() {
  ...
  const groupVal = document.getElementById('hist-req-group-filter')?.value || '';
  let items = inboxReqHistory;
  if (distVal)  items = items.filter(e => (e.submitter_district || '') === distVal);
  if (groupVal) items = items.filter(e => (e.submitter_group || '') === groupVal);
  ...
  el.innerHTML = items.map(e => `
    <div class="card" style="margin-bottom:0;">
      <div class="card-body" style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;">
        <div style="flex:1;min-width:0;">
          <div class="font-semibold" style="font-size:0.95rem;margin-bottom:0.25rem;">${escHtml(e.title)}</div>
          <div class="text-muted text-sm">From: ${escHtml(e.submitter_name || '—')}${e.submitter_group ? ' · ' + escHtml(e.submitter_group) : ''}${e.submitter_district ? ' · ' + escHtml(e.submitter_district) : ''}</div>
      ...
      <div class="card-body">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;margin-bottom:0.75rem;">
          <div>
            ...
            <div class="text-muted text-sm">From: ${escHtml(r.submitter_name || '—')} · ${fmtDate(r.created_at)}</div>
          </div>
...
function renderInboxRptHistory() {
  populateGroupFilter(inboxRptHistory, 'hist-rpt-group-filter');
  filterInboxRptHistory();
}

function filterInboxRptHistory() {
  const el = document.getElementById('inbox-rpt-history-list');
  const groupVal = document.getElementById('hist-rpt-group-filter')?.value || '';
  let items = inboxRptHistory;
  if (groupVal) items = items.filter(r => (r.submitter_group || '') === groupVal);
  if (!items.length) { el.innerHTML = '<div class="card"><div class="card-body text-muted text-sm">No history.</div></div>'; return; }
...
function populateHistoryFilters(items, distId, groupId) {
  const distEl  = document.getElementById(distId);
  const groupEl = document.getElementById(groupId);
  if (!distEl || !groupEl) return;
  const dists  = [...new Set(items.map(i => i.submitter_district).filter(Boolean))];
  const groups = [...new Set(items.map(i => i.submitter_group).filter(Boolean))];
  distEl.innerHTML  = '<option value="">All Districts</option>' + dists.map(d => `<option value="${escHtml(d)}">${escHtml(d)}</option>`).join('');
  groupEl.innerHTML = '<option value="">All Groups</option>'    + groups.map(g => `<option value="${escHtml(g)}">${escHtml(g)}</option>`).join('');
}

function populateGroupFilter(items, groupId) {
  const el = document.getElementById(groupId);
  if (!el) return;
  const groups = [...new Set(items.map(i => i.submitter_group).filter(Boolean))];
  el.innerHTML = '<option value="">All Groups</option>' + groups.map(g => `<option value="${escHtml(g)}">${escHtml(g)}</option>`).join('');
}
...
async function submitCommsRequest() {
  ...
  if (res && res.id) {
    alertEl.innerHTML = '<div class="alert alert-success"><span class="alert-icon">✅</span> Event request submitted.</div>';
...
function clearCommsRequestForm() {
  ...
  document.getElementById('cr-approval-level').value = 'gc';
}
...
function clearCommsReportForm() {
  ...
  document.getElementById('crpt-approval-level').value = 'gc';
}


const SECTION_TITLES = {
  home:             ['Dashboard',          'District overview'],
  users:            ['My Group',           'Manage your group'],
  'send-request':   ['Send Request',       'Submit an event request'],
  'send-report':    ['Send Report',        'Submit a report'],
  'inbox-requests': ['Received Requests',  'Review event requests from group leaders'],
  'inbox-reports':  ['Received Reports',   'Review and forward reports'],
...
function showUsersTab(tab, btn) {
  ...
  const titles = {
    leaders: ['Leaders', 'Manage group leaders in your district'],
    members: ['Members', 'View members in your district'],
  };
