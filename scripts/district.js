...
function renderUsersTable(users) {
  ...
  if (!display.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="text-muted text-sm" style="padding:1rem;">No users in this category.</td></tr>';
    return;
  }
  tbody.innerHTML = display.map(u => {
    ...
    <tr style="${!u.editable ? 'opacity:0.8;' : ''}">
      <td class="td-name">
        <div>${escHtml(displayName)}${!u.editable ? ' <span class="badge badge-neutral" style="font-size:0.7rem;">view only</span>' : ''}</div>
...
function _roleLabel(u) {
  ...
  if (u.group_code) parts.push(u.group_code);
